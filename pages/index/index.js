...
Page({
  ...
  processStart() {
    try {
      const balanced = balance(this.data.input);
      this.setData({
        output: balanced,
        warning: false
      });
      // 如果需要渲染MathJax，可以考虑使用WebView组件加载渲染后的HTML
    } catch (err) {
      this.setData({
        warning: true,
        output: err
...
// 以下是从你提供的 main.js 和 index_script.js 转换而来的函数
...
function balance(inputEqu) {
    try {
        ...
        let leftChems = [];
        let rightChems = [];
        leftchemstr.forEach(element => leftChems.push(new Chemical(element.trim())));
        rightchemstr.forEach(element => rightChems.push(new Chemical(element.trim())));
        let leftElements = [];
        let rightElements = [];
